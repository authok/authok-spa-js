...
export const assertUrlEquals = (
  ...
  if (strict) {
    expect(searchParamsObj).toStrictEqual({
      authokClient: expect.any(String),
      ...queryParams
    });
  } else {
...
export const setupFn = (mockVerify: jest.Mock) => {
  return (config?: Partial<AuthokClientOptions>, claims?: Partial<IdToken>) => {
...
export const loginWithRedirectFn = (mockWindow, mockFetch) => {
  return async (
    ...
    return await authok.handleRedirectCallback(customCallbackUrl);
  };
};
...
export const loginWithPopupFn = (mockWindow, mockFetch) => {
  return async (
    ...
    await authok.loginWithPopup(options, config);
  };
};
...
export const getTokenSilentlyFn = (mockWindow, mockFetch) => {
  return async (
    ...
    return await authok.getTokenSilently(options);
  };
};
