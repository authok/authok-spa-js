...
describe('AuthokClient', () => {
  ...
  describe('getIdTokenClaims', () => {
    it('returns undefined if there is no cache', async () => {
      const authok = setup();
      const decodedToken = await authok.getIdTokenClaims();
      ...
      ({
        ...
        describe(`when ${name}`, () => {
          it('returns the ID token claims', async () => {
            ...
            expect(await authok.getIdTokenClaims()).toHaveProperty('exp');
            expect(await authok.getIdTokenClaims()).not.toHaveProperty('me');
            expect(await authok.getIdTokenClaims({})).toHaveProperty('exp');
            expect(
              await authok.getIdTokenClaims({ audience: 'default' })
            ).toHaveProperty('exp');
            expect(
              await authok.getIdTokenClaims({ scope: 'foo' })
            ).toHaveProperty('exp');
            expect(
          ...
          it('returns the ID token claims with custom scope', async () => {
            ...
            await login(authok, { scope: 'scope3' });

            expect(
              await authok.getIdTokenClaims({ scope: 'scope1 scope2 scope3' })
            ).toHaveProperty('exp');
          });

          describe('when using refresh tokens', () => {
            it('returns the ID token claims with offline_access', async () => {
              const authok = setup({ scope: 'foo', useRefreshTokens: true });
              await login(authok);

              expect(
                await authok.getIdTokenClaims({ scope: 'foo offline_access' })
              ).toHaveProperty('exp');
            });
